...
export const TimerProvider = ({ children }) => {
  ...
  // Add effect to log topic changes
  useEffect(() => {
    console.log('TimerContext - selectedTopic changed:', selectedTopic);
    if (selectedTopic) {
      console.log('TimerContext - Topic ID type:', typeof selectedTopic.id);
      console.log('TimerContext - Topic ID value:', selectedTopic.id);
      console.log('TimerContext - Topic ID string representation:', String(selectedTopic.id));
    }
  }, [selectedTopic]);
  ...
  const startTimer = async (initialTime, isBreakSession = false) => {
    ...
    try {
      ...
      // Log the current selected topic
      console.log('TimerContext - Starting timer with topic:', selectedTopic);
      console.log('TimerContext - Topic ID type:', typeof selectedTopic?.id);
      console.log('TimerContext - Topic ID value:', selectedTopic?.id);
      console.log('TimerContext - Topic ID string representation:', String(selectedTopic?.id));
      ...
      if (selectedTopic?.id) {
        ...
        console.log('TimerContext - Converting topic ID to string:', idStr);
        
        // UUID format validation
        const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        const isValidUUID = uuidPattern.test(idStr);
        console.log('TimerContext - Topic ID format check:', isValidUUID);
      ...
      console.log('TimerContext - Final topic ID:', topicId);
      ...
      console.log('TimerContext - Creating session with data:', sessionData);
      ...
      console.log('TimerContext - Session created successfully:', data);
  ...
  const startPomodoro = async (settings = pomodoroSettings) => {
    ...
    const startTime = new Date();
    setSessionStartTime(startTime);
    setTime(settings.studyDuration);
    setIsRunning(true);
  ...
  const handlePomodoroComplete = () => {
    ...
    if (status.isStudy) {
      ...
      setTime(pomodoroSettings.studyDuration);
    }
  };
